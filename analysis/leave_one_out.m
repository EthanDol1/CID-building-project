...
totalTime = 0;
%% Fold-invariant setup
% None of this depends on which diary is left out, so it is built once here
% rather than 45 times inside the loop. That removes 90 spreadsheet reads,
% 45 rebuilds of E_num, and 45 x 820 cellfun/contains calls.
...
% Columns 1-6 of the master list. Columns 5 and 6 come from the full diary
% analysis loaded above, not from the fold, so they are fold-invariant too.
masterListBase = [num2cell(V_num) V nodePatterns nodeDiaryPatterns];

list1 = strings(length(V),1);
list2 = strings(length(V),1);
for i = 1:length(V_num)
    if masterListBase(i,4) == "n/a"
        list1(i) = 0;
        list2(i) = 0;
    else
        list1(i) = compose("%.8g",avg_self_loops(find(masterListBase(i,4) == diaryPatterns)));
        list2(i) = compose("%.8g",avg_entries(find(masterListBase(i,4) == diaryPatterns)));
    end
end
masterListBase = [masterListBase list1 list2];

% Row of diaryData that each node's pattern maps to, 0 for "n/a" nodes.
% diaryData(:,1) is diaryPatterns every fold, so this lookup never changes.
diaryRow = zeros(length(V),1);
for j = 1:length(V)
    if nodeDiaryPatterns(j) ~= "n/a"
        diaryRow(j) = find(cellfun(@(p) contains(nodeDiaryPatterns{j}, p), diaryPatterns),1);
    end
end

% Lookup from node number to its column in diaryPatterns, replacing the two
% find() calls the use analysis used to make for every timestep of every
% trajectory. A 0 marks a node whose pattern is "n/a" (rooms and suites);
% those timesteps contributed to no column and are dropped below.
nodeDiaryIdx = zeros(length(V),1);
for v = 1:length(V)
    di = find(diaryPatterns == nodeDiaryPatterns(v));
    if ~isempty(di)
        nodeDiaryIdx(v) = di;
    end
end
...
for l = 1:45
    ...
    % Columns 7-18: self-loop probabilities and entry counts per time of day.
    % Built as one preallocated block and concatenated once, rather than
    % growing masterList twelve times.
    listBlock = zeros(length(V),12);
    for c = 1:12
        for j = 1:length(V)
            if diaryRow(j) > 0
                listBlock(j,c) = diaryData(diaryRow(j), c+1);
            end
        end
    end
    masterList = [masterListBase compose("%.8g",listBlock)];
    ...
    for k = 1:length(room_index)
        ...
        % Markov chains used in running the simulations

        mc_em = dtmc(T_em','StateNames',V);
        mc_m  = dtmc(T_m', 'StateNames',V);
        mc_a  = dtmc(T_a', 'StateNames',V);
        mc_ee = dtmc(T_ee','StateNames',V);
        mc_le = dtmc(T_le','StateNames',V);
        mc_n  = dtmc(T_n', 'StateNames',V);
        ...
        for i = 1:n
            ...
            X0 = simulate(mc_em,ts,'X0',x0);
        
            x1 = zeros(410,1);
            x1(X0(end)) = 1;
            X1 = simulate(mc_m,ts+1,'X0',x1);
            X1 = X1(2:end);
        
            x2 = zeros(410,1);
            x2(X1(end)) = 1;
            X2 = simulate(mc_a,ts+1,'X0',x2);
            X2 = X2(2:end);
        
            x3 = zeros(410,1);
            x3(X2(end)) = 1;
            X3 = simulate(mc_ee,ts+1,'X0',x3);
            X3 = X3(2:end);
        
            x4 = zeros(410,1);
            x4(X3(end)) = 1;
            X4 = simulate(mc_le,ts+1,'X0',x4);
            X4 = X4(2:end);
        
            x5 = zeros(410,1);
            x5(X4(end)) = 1;
            X5 = simulate(mc_n,ts+1,'X0',x5);
            X5 = X5(2:end);
        ...
        patIdx = nodeDiaryIdx(trajectoryMatrix);   % 96 x n pattern columns
        patIdx(trajectoryMatrix == x) = 1;         % time in my_room counts to column 1
        repIdx = repmat(1:n, 96, 1);               % 96 x n trajectory numbers

        counted = patIdx > 0;                      % drop the "n/a" nodes
        diaryPatternUse = accumarray([repIdx(counted) patIdx(counted)], 0.25, ...
            [n length(diaryPatterns)]);
