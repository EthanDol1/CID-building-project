...
fm = zeros(96,594,200);
fm_old = zeros(96,594,200);
ep = zeros(96,594,200);
ep_old = zeros (96,594,200);
rp = zeros(96,594,200);
rt = zeros(96,594,200);
rt_old = zeros(96,594,200);

% The data loaded in by this commented out code was generated with models
% with significant bugs.

load("fm_data1.mat"); fm_old(:,:,1:30)    = fm_simulation_matrix;
load("fm_data2.mat"); fm_old(:,:,31:60)   = fm_simulation_matrix;
load("fm_data3.mat"); fm_old(:,:,61:90)   = fm_simulation_matrix;
load("fm_data4.mat"); fm_old(:,:,91:200)  = fm_simulation_matrix;

% load("ep_data1.mat"); ep_old(:,:,1:30)    = ep_simulation_matrix;
% load("ep_data2.mat"); ep_old(:,:,31:60)   = ep_simulation_matrix;
% load("ep_data3.mat"); ep_old(:,:,61:90)   = ep_simulation_matrix;
% load("ep_data4.mat"); ep_old(:,:,91:200)  = ep_simulation_matrix;

load("rt_data1.mat"); rt_old(:,:,1:30)    = rt_simulation_matrix;
load("rt_data2.mat"); rt_old(:,:,31:90)   = rt_simulation_matrix;
load("rt_data3.mat"); rt_old(:,:,91:120)  = rt_simulation_matrix;
load("rt_data4.mat"); rt_old(:,:,121:200) = rt_simulation_matrix;

load("fm_data.mat"); fm(:,:,:) = fm_simulation_matrix;
load("ep_data.mat"); ep(:,:,:) = ep_simulation_matrix;
load("ep_old_data.mat"); ep_old(:,:,:) = ep_simulation_matrix;
...
figure
b = bar([1 2 3], mean_se, 'FaceColor', 'flat');
b.CData = modelColors(numel(mean_se));
hold on

er = errorbar([1 2 3], mean_se, -1*std_se, std_se);
er.Color = [0 0 0];
er.LineStyle = 'none';
er.LineWidth = 1;
...
x = [ones(nIterations,1) 2*ones(nIterations,1) 3*ones(nIterations,1)];
...
ax.FontSize = 20;
xticklabels(modelLabels)
xlim([0.5 3.5])       % keep the outer swarms clear of the axes box
box on
